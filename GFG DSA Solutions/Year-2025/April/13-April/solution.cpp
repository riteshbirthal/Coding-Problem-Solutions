...
class Solution {
  public:
    void visitGraph(Node* node, map<Node*, Node*>& visited){
        visited[node] = new Node(node->val, node->neighbors);
        int n = visited[node]->neighbors.size();
        for(int i = 0; i < n; i++){
            if(visited.find(visited[node]->neighbors[i])==visited.end()){
                visitGraph(visited[node]->neighbors[i], visited);
            }
            visited[node]->neighbors[i] = visited[visited[node]->neighbors[i]];
        }
        return ;
    }
    
    Node* cloneGraph(Node* node) {
        map<Node*, Node*> visited, visited2;
